...
/* Hybrid genres (rock, shoegaze, post-punk, metal, industrial-rock…) get a
   blend: live kits and amps alongside synths, tape and pedals. Full atom
   coverage — before v4.3 these genres fell back to organic, then techno
   vocabulary for ~71 of the ~92 atoms. */
const PEDALS = ["fuzz-pedal", "overdrive-pedal", "distortion-pedal", "tape-echo-pedal", "spring-reverb-pedal", "tremolo-pedal", "vibrato-pedal", "wah-pedal", "phaser-pedal", "flanger-pedal", "univibe-pedal", "octave-pedal", "boost-pedal", "compressor-pedal", "gate-pedal"];
const SYNTH = ["analog synth", "string machine", "mellotron", "solina strings", "prophet pads", "juno pads", "moog bass", "arp sequence", "ob pads", "polymoog", "modular blip line", "dronescape synth"];
const HYB_AMP = ["cranked stack", "half-stack", "tube head", "combo amp", "amp-and-DI blend", "wall-of-cabs", "isolated cab"];
const HYB_ROOM = ["live room", "rehearsal space", "garage", "concrete room", "wood-panelled studio", "echo chamber", "tiled room", "hall"];
const HYB_TAPE = ["tape-warmed", "saturated-to-tape", "reel-to-reel", "cassette-colored", "valve-driven", "analogue-console", "transformer-heavy"];
const HYB_DRIVE = ["motorik", "krautrock", "post-punk", "shoegaze", "grunge", "new-wave", "art-rock", "noise-rock", "prog", "goth"];

const HYBRID = {
  kick: cat(X(cat(WARM, BIG), ["kick"]), X(["layered acoustic-and-electronic", "sampled-and-live", "reinforced", "triggered-and-real"], ["kick"]), X(HYB_ROOM, ["kick"])),
  hats: cat(X(["crisp", "tight", "washy", "sizzling", "gated", "loose"], ["hi-hat", "hi-hats"]), X(["straight 8th", "driving 16th", "off-beat", "motorik"], ["hi-hat pattern"]), X(["hand-played", "machine-tight", "half-open"], ["hats"])),
  snare: cat(X(["layered", "sampled-and-live", "reinforced", "gated live", "roomy"], ["snare"]), X(cat(WARM, BIG), ["snare"]), X(HYB_ROOM, ["snare crack"])),
  perc: cat(X(["tambourine", "cowbell", "claves", "woodblock", "cabasa", "shaker", "triangle"], ["pattern", "accents"]), X(["metal", "brake-drum", "anvil", "chain"], ["percussion"])),
  toms: cat(X(cat(WARM, BIG), ["toms", "floor tom"]), X(["cascading", "rolling", "thundering", "tribal"], ["tom run"]), X(HYB_ROOM, ["toms"])),
  groove: cat(X(HYB_DRIVE, ["groove", "pocket", "drive"]), X(["locked", "relentless", "surging", "driving", "unstoppable"], ["groove", "momentum"])),
  swing: cat(X(["straight", "pushed", "dragged", "loose-limbed", "machine-tight"], ["feel", "swing"])),
  sync: cat(X(["off-beat", "anticipated", "cross-rhythm", "motorik pulse", "stabbed"], ["syncopation", "accents"])),
  intensity: cat(X(cat(BIG, ["full-band", "wall-of-sound", "stadium", "frenzied"]), ["intensity", "drive", "push"])),
  bassVoice: cat(X(cat(WARM, BIG), BASSAC), X(["sub-reinforced", "DI-and-amp", "synth-doubled", "fuzz-driven", "picked"], ["bass"]), X(PEDALS.slice(0, 5), ["bass tone"])),
  bassMovement: cat(X(["driving", "pulsing", "galloping", "octave-jumping", "pedal-point", "locked-to-kick", "wandering"], ["bass line", "movement"])),
  bassRel: cat(X(["bass locked with the kick", "bass duelling the guitar", "bass anchoring the wall", "bass mirroring the synth", "bass driving under everything"], ["", "in the pocket", "an octave down"])),
  leadVoice: cat(X(cat(WARM, BIG), cat(GTR, KEYS, HORNS, SYNTH)), X(["synth-doubled", "amp-and-DI", "octave-doubled", "feedback-fed"], cat(GTR, KEYS)), X(GTR, HYB_AMP)),
  leadPerf: cat(X(["played with", "performed with", "articulated with"], ["fierce vibrato", "gliding bends", "hammer-ons", "tremolo picking", "palm muting", "feedback sustain", "whammy dives", "double-stops", "slid harmonics", "wide bends"]), X(PEDALS.slice(0, 8), ["lead tone"])),
  harmony: cat(X(cat(WARM, BIG), ["chords", "harmony", "voicings"]), X(["power-chord", "open-chord", "spread", "shoegaze-wall", "pad-backed"], ["harmony", "stabs"]), X(SYNTH, ["pads", "harmony"])),
  arpeggio: cat(X(["fingerpicked", "sequenced", "cascading", "chiming", "arpeggiated", "picked-through"], ["arpeggio", "figure"])),
  contour: cat(X(["rising", "falling", "arch", "leaping", "circling", "soaring", "diving"], ["melodic contour", "phrase shape"])),
  rhythm: cat(X(["straight 8th", "straight 16th", "triplet", "dotted", "syncopated", "double-time"], ["phrasing", "melody rhythm"])),
  technoDrive: cat(X(cat(BIG, HYB_DRIVE, ["locomotive", "freight-train", "runaway"]), ["drive", "momentum", "push"])),
  technoAcid: cat(X(["fuzzed", "wailing", "screaming", "swirling", "howling"], ["guitar line", "organ line", "synth-and-string line", "lead line"])),
  technoTexture: cat(X(cat(HYB_TAPE, ["shoegaze", "reverb-drenched", "shimmer-drenched", "feedback-laced"]), ["texture", "wash", "grain"])),
  technoRave: cat(X(["organ", "brass", "string", "guitar-stack", "synth-stack"], ["stabs", "hits"])),
  technoIndustrial: cat(X(["floor-tom", "metal-percussion", "hammered", "stomp-and-clap", "chain", "brake-drum"], ["rhythm", "hits"])),
  filterType: cat(X(["wah-pedal", "tone-knob", "cocked-wah", "synth filter", "muting-and-opening"], ["filtering", "sweep"])),
  envelopeType: cat(X(["hard-attack", "slow-bloom", "swelling", "plucked", "gated", "bowed-in"], ["envelope", "attack"])),
  lfoType: cat(X(["tremolo", "vibrato-arm", "rotary", "synth LFO", "flutter"], ["0.5Hz", "2Hz", "4Hz", "6Hz", "fast", "slow"], ["modulation"])),
  distortionType: cat(X(cat(PEDALS.slice(0, 6), ["overdriven tube", "cranked valve", "console-driven"]), ["saturation", "drive"])),
  reverbType: cat(X(HYB_ROOM, ["reverb", "ambience"]), X(["plate", "spring", "hall", "shimmer"], ["reverb"])),
  delayType: cat(X(["tape echo", "analog delay", "digital delay", "slapback", "dotted delay"], ["1/8", "1/4", "120ms", "250ms", "dotted"])),
  sidechainType: cat(X(["pumping", "ducking-to-the-kick", "breathing", "band dynamics"], ["", "on the downbeat"])),
  stereoType: cat(X(["stereo-pair", "spaced-omni", "mid-side", "XY-pair", "wide-band"], ["imaging", "stage"])),
  fxChain: cat(X(["guitar", "bass", "synth", "kit", "bus"], ["→"], PEDALS.slice(0, 8).concat(["amp", "tape", "console", "room", "master"]))),
  soundIntensity: cat(X(cat(BIG, ["full-throated", "wide-open", "wall-of-sound"]), ["band intensity", "performance energy"])),
  chordProg: cat(["i–bVI–bIII–bVII", "i–iv–v–i", "I–V–vi–IV", "I–bVII–IV–I", "vi–IV–I–V", "i–bVII–bVI–bVII", "I–IV–V–IV", "chromatic descent", "pedal-drone changes", "modal i–bVII", "power-chord riff changes", "rising sequence"]),
  rhythmPattern: cat(X(["motorik", "four-on-the-floor", "backbeat", "shuffle", "two-step", "blast-pattern", "half-time", "post-punk"], ["pattern"])),
  mixDensity: cat(X(cat(BIG, ["wall-of-guitars", "layered-band", "dense-overdub"]), ["arrangement density"])),
  mixEnergy: cat(X(BIG, ["band energy", "forward drive"])),
  mixSpace: cat(X(HYB_ROOM, ["space", "depth"])),
  mixGlue: cat(X(["console-glued", "tape-glued", "bus-compressed", "room-glued"], ["cohesion", "blend"])),
  mixPunch: cat(X(cat(BIG, ["transient", "stick-attack", "pick-attack"]), ["punch", "impact"])),
  masterDrive: cat(X(HYB_TAPE, ["master drive", "bus colour"])),
  masterLoudness: cat(X(["-7 LUFS", "-8 LUFS", "-9 LUFS", "-10 LUFS", "concert-level", "club-level"], ["master level"])),
  masterColor: cat(X(cat(HYB_TAPE, WARM), ["master colour"])),
  masterChain: cat(["console EQ → tape → bus comp → limiter", "guitar bus → tape → vari-mu → limiter", "Pultec EQ → FET comp → tape → limiter", "mix bus → transformer → opto comp → limiter"]),
  filterCutoff: cat(X(["tone rolled back", "tone wide open", "wah heel-down", "wah toe-down", "filter swept open"], ["", "through the phrase"])),
  filterResonance: cat(X(["singing", "nasal", "throaty", "biting", "whistling"], ["resonance", "formant"])),
  eqType: cat(X(["Pultec-style", "console-strip", "mid-forward", "air-band", "tilt", "graphic"], ["EQ"])),
  compressionType: cat(X(["opto", "vari-mu", "FET", "tube", "parallel", "bus"], ["compression"])),
  saturationType: cat(X(cat(HYB_TAPE, PEDALS.slice(0, 3)), ["saturation"])),
  sidechainCurve: cat(X(["pumping", "breathing", "ducking", "glued"], ["dynamic curve"])),
  stereoImage: cat(X(["stage-left", "stage-right", "centre-forward", "full-stage", "wall-wide"], ["placement", "image"])),
  stereoWidth: cat(X(["natural", "wide-stage", "wall-of-sound", "close-band", "front-row"], ["width"])),
  spatialDepth: cat(X(HYB_ROOM, ["depth", "front-to-back staging"])),
  spatialMovement: cat(X(["walking across the stage", "rotating speaker", "swelling forward", "feedback drifting"], ["", "between phrases"])),
  stereoEnhance: cat(X(["stereo-pair", "room-mic blend", "hall-return", "ambience-bus"], ["widening"])),
  reverbSize: cat(X(HYB_ROOM, ["size"])),
  reverbDecay: cat(X(["0.9s", "1.4s", "2.2s", "3.5s", "5s"], ["decay"])),
  modSource: cat(X(["expression pedal", "vibrato arm", "leslie speed", "synth LFO", "foot-switch"], ["control"])),
  modDest: cat(X(["tone", "vibrato depth", "amp tremolo", "reverb send", "dynamics", "pitch bend"], ["target"])),
  modRate: cat(X(["slow", "medium", "fast", "accelerating", "phrase-length"], ["modulation rate"])),
  modDepth: cat(X(["moderate", "deep", "full-range", "wide"], ["modulation depth"])),
  textureLayer: cat(X(cat(HYB_ROOM, HYB_TAPE), ["texture layer", "ambience bed"])),
  grainType: cat(X(cat(HYB_TAPE, ["vinyl", "cassette hiss", "amp hiss", "room-noise"]), ["grain"])),
  shimmerType: cat(X(["cymbal", "chime-guitar", "glockenspiel", "string-harmonic", "bell-tone"], ["shimmer", "sparkle"])),
  atmosphereType: cat(X(HYB_ROOM, ["atmosphere", "air"])),
  ghostNotes: cat(X(["brushed", "stick-tip", "finger", "palm", "left-hand"], ["ghost notes"])),
  humanizeType: cat(X(["live-band", "hand-played", "locked-to-click", "breathing", "pushed"], ["timing"])),
  pocketType: cat(X(["behind-the-beat", "ahead-of-the-beat", "dead-centre", "driving", "laid-back"], ["pocket"])),
  ornamentType: cat(X(["grace-note", "slide", "fall-off", "doit", "scoop", "shake", "bend"], ["ornament"])),
  vibratoType: cat(X(["wide", "fast", "delayed", "arm", "finger", "feedback-fed"], ["vibrato"])),
  portamentoType: cat(X(["slide", "glissando", "bend-up", "bend-down", "whammy-dive"], ["into the note", "between phrases"])),
  scaleRun: cat(X(["blues", "pentatonic", "modal", "chromatic", "diatonic", "phrygian", "harmonic-minor"], ["run", "lick"])),
  intervalLeap: cat(X(["octave", "fifth", "sixth", "seventh", "ninth", "tenth"], ["leap"])),
  voicingType: cat(X(["close", "open", "drop-2", "spread", "power-chord", "cluster"], ["voicing"])),
  inversionType: cat(X(["root-position", "first-inversion", "second-inversion", "slash-bass", "pedal-bass"], ["voicing"])),
  tensionType: cat(X(["b9", "#11", "13", "add9", "sus4", "6/9"], ["tension"])),
  resolutionType: cat(X(["authentic", "plagal", "deceptive", "blues", "modal", "pedal-hold"], ["resolution"])),
  delayTime: cat(X(["slapback 90ms", "tape 180ms", "quarter-note", "dotted-eighth", "hall pre-delay"], [""])),
  delayFeedback: cat(X(["single repeat", "two repeats", "three repeats", "long trail", "damped"], ["echo"])),
  sectionDensity: cat(X(cat(BIG, ["full-band", "stripped-to-rhythm", "guitars-only", "synths-only"]), ["section density"])),
  rideType: cat(X(["bell-heavy", "washy", "sizzle", "crashing", "rivet"], ["ride cymbal"])),
  crashType: cat(X(cat(BIG, ["china", "splash", "orchestral"]), ["crash", "cymbal"])),
  clapLayer: cat(X(["hand-clap", "layered clap", "stomp", "tambourine"], ["stack", "layer"])),
  percFill: cat(X(["tom", "snare", "cymbal", "metal-percussion"], ["fill", "break"])),
  fxType: cat(X(["tape stop", "reverse swell", "cymbal swell", "feedback squeal", "guitar dive", "amp hum swell"], [""])),
  transitionType: cat(X(["drum fill", "feedback swell", "filter sweep", "cymbal roll", "pick slide"], ["transition"])),
  riserType: cat(X(["cymbal roll", "feedback build", "string ascent", "noise swell", "snare crescendo"], [""])),
  impactType: cat(X(cat(BIG, ["orchestral", "gong", "timpani", "bass-drum"]), ["hit", "impact"])),
  energyCurve: cat(X(["slow-burn", "wall-of-sound climb", "step-climb", "cinematic swell", "relentless ramp"], ["arc"])),
  buildType: cat(X(["8-bar", "16-bar", "guitar-led", "drum-led", "synth-led", "ensemble"], ["build"])),
  dropType: cat(X(["full-band entry", "wall-of-sound entry", "half-time", "stop-time", "feedback slam entry"], [""])),
  chopType: cat(X(["stop-time", "stab", "hit-and-hold", "break", "tag"], ["figure"]))
};
